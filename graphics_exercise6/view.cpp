...
void startup()
{
	...
	sc.plane = new VBOPlane(15.0f, 10.0f, 10, 10);
	
	sc.view = LookAt(vec3(0.0f, 1.0f, 5.0f), 
		vec3(0.0f, 0.0f, 0.0f), 
		vec3(0.0f, 1.0f, 0.0f));

	float cameraZ = sc.height * 0.5 / tan(radians(0.5*80.0));
...
void render()
{
	...
	mat4 model_base1;
	model_base1 = translate(mat4(1.0), vec3(0.0, 3.0, 0.0));
	model_base1 = rotate(model_base1, (float)radians(-90.0), vec3(1.0, 0.0, 0.0));
	model_base1 = scale(model_base1, vec3(0.5, 0.5, 0.5));

	glUniformMatrix4fv(sc.m_loc, 1, GL_FALSE, value_ptr(model_base1));
	glUniform4f(sc.color_loc, 0.3, 0.0, 0.5, 1.0);
	sc.teapot->render();

	model_base1 = translate(model_base1, vec3(1.0, 3.0, 0.0));
	model_base1 = scale(model_base1, vec3(1.0, 1.0, 1.0));
	glUniformMatrix4fv(sc.m_loc, 1, GL_FALSE, value_ptr(model_base1));
	glUniform4f(sc.color_loc, 0.5, 0.0, 0.5, 1.0);
	sc.teapot->render();
