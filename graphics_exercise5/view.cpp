...
void startup()
{
	...
	sc.cube = new VBOCube();
	sc.plane = new VBOPlane(10.0f, 10.0f, 10, 10);
	
	sc.view = lookAt(vec3(0.0f, 1.0f, 5.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f));
...
void render()
{
	...
	x = 5.0 * sin(sc.theta);
	z = 5.0 * cos(sc.theta);
	sc.view = lookAt(vec3(0.0f, 1.0f, z), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f));
	glUniformMatrix4fv(sc.v_loc, 1, GL_FALSE, value_ptr(sc.view));
	...
	// Cube
	mat4 model_base;
	model_base = translate(mat4(1.0), vec3(0.0, 0.25, 0.0));
	model_base = scale(model_base, vec3(1.0, 0.5, 1.0));
	glUniformMatrix4fv(sc.m_loc, 1, GL_FALSE, value_ptr(model_base));
	glUniform4f(sc.color_loc, 0.7, 0.4, 0.6, 1.0);
	sc.cube->render();
